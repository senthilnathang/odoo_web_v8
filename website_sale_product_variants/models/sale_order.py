...
class sale_order(osv.Model):
    ...
    def _website_product_id_change(self, cr, uid, ids, order_id, product_id, qty=0, line_id=None, context=None):
        ...
        search_ids = self.pool.get('product.product').search(
            cr, uid, [('child_ids', '=', product_id)], context=context)
            
        #print "main ma --- product.parent_prod_id.id-----",product.parent_prod_id.id
        #print"first llllllllllllllllllllliiiiiiiinnnnneeee====",line_id
        if search_ids:
            values = self.pool.get('sale.order.line').product_id_change(cr, SUPERUSER_ID, [],
                pricelist=so.pricelist_id.id,
                product=search_ids[0],
                partner_id=so.partner_id.id,
                fiscal_position=so.fiscal_position.id,
                qty=qty,
                context=context
            )['value']
            #prod_obj = product.browse(
            #cr, uid, competitor_prod_id, context=context)
            #if product.competitor_prod_id:
            #product_neni Competitor 
            values['competitor_prod_id']= product_id
            values['product_id'] = search_ids[0]
        else:
            values = self.pool.get('sale.order.line').product_id_change(cr, SUPERUSER_ID, [],
                pricelist=so.pricelist_id.id,
                product=product_id,
            ...
            #print"else ma ave 6????@@@@@@@@@@@@@@"
        #print"above if llllllllllllllllllllliiiiiiiinnnnneeee====",line_id
        if line_id:
            line = self.pool.get('sale.order.line').browse(cr, SUPERUSER_ID, line_id, context=context)
            values['name'] = line.name
            #print"product.description_sale or product.name"
            #print"product.description_sale or product.name=====>>>"
        else:
            product_p = self.pool.get('product.product').browse(cr, uid, product_id, context=context)
            values['name'] = product_p.description_sale or product_p.name
            #print"product.description_sale or product.name",product_p.description_sale 
            #print"product.description_sale or product.name=====>>>",product_p.name
        #values['competitor_prod_id']= product.parent_prod_id.id
    ...
    def _cart_update(self, cr, uid, ids, product_id=None, line_id=None, add_qty=0, set_qty=0, context=None, **kwargs):
        ...
        #print "FFFFFFFFFFFFFFFFFFFFFFFFFFFFF", product_id
        quantity = 0
        for so in self.browse(cr, uid, ids, context=context):
            if line_id != False:
