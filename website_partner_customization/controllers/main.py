...
import openerp.addons.website_crm_partner_assign.controllers.main as website_partner_custom
...
class WebsiteCrmPartnerlicense(website_partner_custom.WebsiteCrmPartnerAssign):
	...
	'/partners/license/<model("license.code"):license>',
	'/partners/license/<model("license.code"):license>/page/<int:page>',

        '/partners/country/<model("res.country"):country>',
        '/partners/country/<model("res.country"):country>/page/<int:page>',

        '/partners/grade/<model("res.partner.grade"):grade>/country/<model("res.country"):country/<model("license.code"):license>',
        '/partners/grade/<model("res.partner.grade"):grade>/country/<model("res.country"):country>/page/<model("license.code"):license>/page/<int:page>',
    ], type='http', auth="public", website=True)
    def partners(self, country=None, grade=None, license=None, page=0, **post):
	...
	# group by License
        license_domain = list(base_partner_domain)
        ...
        if country:
            license_domain += [('country_id', '=', country.id)]
        #if license:
            #license_domain += [('license_code', '=', license.id)]
        licenses = partner_obj.read_group(
            request.cr, SUPERUSER_ID, license_domain, ["id", "license_code"],
            groupby="license_code", orderby="license_code", context=request.context)
        licenses_partners = partner_obj.search(
            request.cr, SUPERUSER_ID, license_domain,
            context=request.context, count=True)
        # flag active License
        for license_dict in licenses:
	    #print"license.code===>>>",license
	    #print"license_dict['license_code']===>>>",license_dict['license_code']
            license_dict['active'] = license and license_dict['license_code'] and license_dict['license_code'][0] == license.id
        licenses.insert(0, {
            'license_code_count': licenses_partners,
            'license_code': (0, _("All Licenses")),
            'active': bool(license is None),
        })
        # group by grade
	...
	if license:
	    base_partner_domain += [('license_code', '=', license.id)]
